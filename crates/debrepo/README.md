...
`debrepo` is **signing-agnostic** by design: it returns the `Release` text so the
caller signs it (`InRelease` / `Release.gpg`) with whatever PGP implementation it
prefers. No async runtime, no native dependencies.

> Part of the [ArtifactX](https://github.com/jamesarch/artifactx) workspace, but
...
all you need is "turn a pool of `.deb`s into a signed, servable repo," `debrepo`
...
```rust
use debrepo::{build_apt, ReleaseMeta};
use std::path::Path;
...
Early but tested (`cargo test -p debrepo`). Not yet implemented: incremental
