...
fn import_accepts_aptly_hash_prefixed_deb_filenames() {
    ...
    std::fs::write(
        ...
        "Origin: Qgnet Repository\nLabel: Qgnet Repository\nSuite: oldstable\nCodename: bullseye\n",
    )
    .unwrap();
    ...
    assert!(config.contains("origin = \"Qgnet Repository\""));
    assert!(config.contains("label = \"Qgnet Repository\""));
    ...
    assert!(release.contains("Origin: Qgnet Repository"));
    assert!(release.contains("Label: Qgnet Repository"));
...
fn package_list_api_supports_search_filters() {
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path().join("repo");
    let first = tmp.path().join("api-wss_1.0-1_amd64.deb");
    let second = tmp.path().join("api-other_1.0-1_amd64.deb");
    write_deb(&first, "api-wss", "1.0-1", "amd64");
    ...
    let response: serde_json::Value =
        reqwest::blocking::get(format!("{base}/api/v1/packages?q=wss&apt=true&scope=main"))
            .unwrap()
            .error_for_status()
            .unwrap()
            .json()
    ...
    assert_eq!(packages[0]["name"], "api-wss");
...
fn search_cli_filters_pool_entries_and_emits_json() {
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path().join("repo");
    let keep = tmp.path().join("wss-agent_2.0-1_amd64.deb");
    let other = tmp.path().join("qgproxy_1.0-1_amd64.deb");

    arx_ok(&["init", root.to_str().unwrap(), "--no-key"]);
    write_deb(&keep, "wss-agent", "2.0-1", "amd64");
    write_deb(&other, "qgproxy", "1.0-1", "amd64");
    arx_ok(&[
        "add",
        keep.to_str().unwrap(),
        other.to_str().unwrap(),
    ...
    assert_eq!(packages[0]["name"], "wss-agent");
    ...
    assert!(stdout.contains("qgproxy\t1.0-1\tamd64\tmain\tapt"));
    assert!(!stdout.contains("wss-agent"));
}
