...
- `crates/debrepo` — apt repo generator — **MIT OR Apache-2.0**.
- `crates/pack` — packager — **MIT OR Apache-2.0**.
- `crates/arx` — the CLI/server (links `createrepo_rs`) — **GPL-2.0-or-later**.

The GPL boundary stops at `arx`. `debrepo` and `pack` never depend on the GPL code.

## Consequences

- Good: anyone can `cargo add debrepo`/`pack` into a closed-source or
...
- **One GPL crate.** Simplest, but traps `debrepo`/`pack` behind GPL — kills their
  reuse, which is half their point (esp. `pack` as the embeddable moat).
